...
interface DebtDocumentsProps {
  documents: DebtDocument[]
  onUpdateDocuments: (documents: DebtDocument[]) => void
}

export function DebtDocuments({ documents, onUpdateDocuments }: DebtDocumentsProps) {
  const [category, setCategory] = useState('')
  const [type, setType] = useState('')
  const { uploadFile } = useFileUpload('debtDocuments')

  const handleFileSelect = async (file: File) => {
    try {
      const url = await uploadFile(file)
      
      const newDocument: DebtDocument = {
        fileId: url,
        fileName: file.name,
